...
namespace application.services
{
    ...
    /// 新人券
    /// </summary>
    public class TicketService : bases.BaseServiceLfex, ITicketService
    {
        ...
        public TicketService(IOptionsMonitor<ConnectionStringList> connectionStringList, CSRedisClient redisClient, IOptionsMonitor<Models.TicketConfig> monitor, IOptionsMonitor<Models.AppSetting> options) : base(connectionStringList)
        {
        ...
        public async Task<MyResult<object>> InitAccount(long userId)
        {
            ...
            {
                Yoyo.Core.SystemLog.Debug($"初始化股新人券账户发生错误\r\n插入语句：{InsertSql}\r\n判断语句：{SelectSql}", ex);
        ...
        public async Task<MyResult<TicketModel>> TicketPage(long userId)
        {
            ...
            TicketModel accInfo = await base.dbConnection.QueryFirstOrDefaultAsync<TicketModel>(SelectSql);
            if (accInfo == null)
            {
            ...
            {
                accInfo.WatchCunt = base.dbConnection.QueryFirstOrDefault<Int32?>($"SELECT COUNT(RecordId) FROM {RecordTableName} WHERE ModifyType = 3 AND TO_DAYS(NOW()) = TO_DAYS(ModifyTime) AND AccountId = @AccountId;", new { accInfo.AccountId }) ?? 0;
            }
        ...
        public async Task<MyResult<UserAccountTicket>> TicketInfo(long userId)
        {
            ...
            String SelectSql = $"SELECT * FROM `{AccountTableName}` WHERE `UserId` = {userId} LIMIT 1";
            UserAccountTicket accInfo = await base.dbConnection.QueryFirstOrDefaultAsync<UserAccountTicket>(SelectSql);
        ...
        public async Task<MyResult<Object>> TicketSwitch(Int64 UserId)
        {
            ...
            Int32 Rows = dbConnection.Execute(UpdateSql.ToString(), UpdateParam);
            if (Rows > 0)
            {
                result.Data = accInfo;
                return result;
        ...
        {
            MyResult<Object> result = new MyResult<object>();
            if (TicketConf.SubscribeTotal == 0) { return result.SetStatus(ErrorCode.InvalidData, "新人券已兑完"); }
            ...
            Int32 TicketTotal = base.dbConnection.QueryFirstOrDefault<Int32>($"SELECT SUM(Balance) FROM {AccountTableName};");
            if (TicketTotal >= TicketConf.SubscribeTotal) { return result.SetStatus(ErrorCode.InvalidData, "新人券已兑完~"); }

            Int32 DayTotal = base.dbConnection.QueryFirstOrDefault<Int32>($"SELECT COUNT(RecordId) FROM {RecordTableName} WHERE ModifyType = 1 AND TO_DAYS(NOW()) = TO_DAYS(ModifyTime);");
            if (DayTotal >= TicketConf.DayShares) { return result.SetStatus(ErrorCode.InvalidData, "今日新人券已兑完,明天早点来哦~"); }
            if ((TicketConf.DayShares - DayTotal) < exchange.Shares)
            {
                return result.SetStatus(ErrorCode.InvalidData, $"本日可兑新人券，仅剩{TicketConf.DayShares - DayTotal}份~");
            }
            ...
            Decimal UserDevote = base.dbConnection.ExecuteScalar<Decimal>($"SELECT IFNULL(SUM(Devote),0) AS Devote FROM yoyo_member_devote WHERE UserId={UserInfo.Id}") + (UserInfo.Golds == null ? 0 : UserInfo.Golds.Value);
            ...
            if (UserDevote < tmpSysLevel.Claim)
            {
                ...
                base.dbConnection.Execute($"UPDATE `user` SET `level`='{UserLevel}' WHERE Id={UserInfo.Id}");
            }
            ...
            CandyRecordParams.Add("CandyDesc", $"兑换新人券: {exchange.Shares.ToString()}份", DbType.String);
            ...
            PeelRecordParams.Add("PeelDesc", $"兑换新人券: {exchange.Shares.ToString()}份", DbType.String);
            ...
            return result.SetStatus(ErrorCode.InvalidData, "兑换新人券失败");
        }
        ...
        public async Task<MyResult<Object>> UseTicket(long UserId)
        {
            ...
            try
            {
                ...
                if (payOrder != null) { return result.SetStatus(ErrorCode.InvalidData, "快去填写认证信息吧"); }
                #endregion

                var UserInfo = base.dbConnection.QueryFirstOrDefault<User>("SELECT * FROM `user` WHERE id = @UserId;", new { UserId = UserId });
                if (UserInfo == null) { return result.SetStatus(ErrorCode.ErrorSign, "请重新登陆后,重试"); }
                var ReUser = base.dbConnection.QueryFirstOrDefault<User>("SELECT * FROM `user` WHERE `mobile` = @InviterMobile;", new { InviterMobile = UserInfo.InviterMobile });
                String SelectSql = $"SELECT * FROM `{AccountTableName}` WHERE `UserId` = {ReUser.Id} LIMIT 1;";
                UserAccountTicket accInfo = await base.dbConnection.QueryFirstOrDefaultAsync<UserAccountTicket>(SelectSql);
                if (accInfo == null || accInfo.State == 0) { return result.SetStatus(ErrorCode.InvalidData, "推荐人未开启新人券功能"); }

                MyResult<object> DeductRult = await ChangeAmount(ReUser.Id, -1, TicketModifyType.TICKET_USED, false, UserInfo.Name, "1");
                if (!DeductRult.Success || !(Boolean)DeductRult.Data)
                {
                    return result.SetStatus(ErrorCode.InvalidData, "使用新人券失败");
                }
                ...
                if (orderGame == null)
                {
                    ...
                    rows = base.dbConnection.Execute(InsertSql.ToString(), SqlParam);
                ...
                else
                {
                    ...
                    rows = base.dbConnection.Execute(UpdateSql.ToString(), SqlParam);
            ...
            {
                Yoyo.Core.SystemLog.Debug("新人券异常==>", ex);
        ...
        public async Task<MyResult<Object>> TicketTask(long UserId)
        {
            ...
            Int32 DayTotal = base.dbConnection.QueryFirstOrDefault<Int32>($"SELECT COUNT(RecordId) FROM {RecordTableName} WHERE ModifyType = 3 AND TO_DAYS(NOW()) = TO_DAYS(ModifyTime) AND AccountId = @AccountId;", new { accInfo.AccountId });
            if (DayTotal >= TicketConf.TaskCount) { return result.SetStatus(ErrorCode.InvalidData, "今日任务已完,明天再来哦~"); }
            ...
            return result.SetStatus(ErrorCode.InvalidData, "任务失败~");
        }
        ...
        public async Task<MyResult<List<UserAccountTicketRecord>>> TicketRecords(QueryModel query)
        {
            ...
            IEnumerable<UserAccountTicketRecord> Records = await base.dbConnection.QueryAsync<UserAccountTicketRecord>($"SELECT * FROM {RecordTableName} WHERE `AccountId`={accInfo.AccountId} ORDER BY RecordId DESC LIMIT {(query.PageIndex - 1) * query.PageSize},{query.PageIndex * query.PageSize}");
        ...
        /// <returns></returns>
        private async Task<MyResult<object>> ChangeAmount(long userId, decimal Amount, TicketModifyType modifyType, bool useFrozen, params string[] Desc)
        {
            MyResult result = new MyResult { Data = false };
            ...
            try
            {
                ...
                #region 验证账户信息
                String SelectSql = $"SELECT * FROM `{AccountTableName}` WHERE `UserId` = {userId} LIMIT 1";
                UserAccount = await base.dbConnection.QueryFirstOrDefaultAsync<UserAccountTicket>(SelectSql);
                if (UserAccount == null) { return result.SetStatus(ErrorCode.InvalidData, "新人券不存在"); }
                if (Amount < 0)
                {
                    if (useFrozen)
                    {
                        if (UserAccount.Frozen < Math.Abs(Amount) || UserAccount.Balance < Math.Abs(Amount)) { return result.SetStatus(ErrorCode.InvalidData, "新人券不足[F]"); }
                    }
                    else
                    {
                        if (UserAccount.Balance < Math.Abs(Amount)) { return result.SetStatus(ErrorCode.InvalidData, "新人券不足[B]"); }
                        if ((UserAccount.Balance - UserAccount.Frozen) < Math.Abs(Amount)) { return result.SetStatus(ErrorCode.InvalidData, "可用新人券不足[F]"); }
                    }
                }
                #endregion
                ...
                using (IDbTransaction Tran = dbConnection.BeginTransaction())
                {
                    try
                    {
                        ...
                        Tran.Rollback();
                        return result.SetStatus(ErrorCode.InvalidData, "新人券变更发生错误");
                    }
                    catch (Exception ex)
                    {
                        Tran.Rollback();
                        Yoyo.Core.SystemLog.Debug($"新人券账户变更发生错误\r\n修改语句：\r\n{EditSQl}\r\n记录语句：{RecordSql}", ex);
                        return result.SetStatus(ErrorCode.InvalidData, "发生错误");
                    }
            ...
            {
                Yoyo.Core.SystemLog.Debug($"新人券变更发生错误\r\n修改语句：\r\n{EditSQl}\r\n记录语句：{RecordSql}", ex);
                return result.SetStatus(ErrorCode.InvalidData, "发生错误");
            }
            finally
            {
