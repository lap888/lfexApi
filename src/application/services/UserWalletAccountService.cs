...
{
    public class UserWalletAccountService : BaseServiceLfex, IUserWalletAccountService
    {
        ...
        public UserWalletAccountService(IOptionsMonitor<ConnectionStringList> connectionStrings, CSRedisClient redis, IAlipay alipay, IOptionsMonitor<Models.AppSetting> monitor) : base(connectionStrings)
        {
        ...
        public async Task<MyResult<object>> InitWalletAccount(long userId)
        {
            ...
            {
                Yoyo.Core.SystemLog.Debug($"初始化钱包账户发生错误\r\n插入语句：{InsertSql}\r\n判断语句：{SelectSql}", ex);
        ...
        public MyResult<object> Recharge(int userId, decimal Amount)
        {

            return new MyResult<object> { Code = -1, Message = "充值通道维护中" };
            // if(userId < 0) { return new MyResult<object> { Code = -1, Message = "用户信息错误" }; }

            // String CacheKey = $"AliAction_Recharge:{userId}";
            // if (RedisCache.Exists(CacheKey)) { return new MyResult<object> { Code = -1, Message = "请勿重复提交" }; }
            // return await CreatePayUrl(userId, Amount, ActionType.CASH_RECHARGE);
        }
        ...
        public async Task<MyResult<object>> Withdraw(int userId, decimal Amount, string TradePwd, string TradeNo)
        {
            ...
            try
            {
                WithdrawLimit = AppSetting.Levels.FirstOrDefault(item => item.Level.Equals(userInfo.Level)).WithdrawLimit;
            }
            catch { }
            ...
            try
            {
                ...
                //if (Push == 1) { result.Data = true; } else { Yoyo.Core.SystemLog.Error($"用户提现=发送订阅失败:{TradeNo}-{userId}-{Amount}"); }
                #endregion
                RedisCache.Set(DayLimitLock, Amount, DateTime.Now.AddDays(1).Date - DateTime.Now);
                Yoyo.Core.SystemLog.Error($"{userId}={Amount}={TradeNo}");

                return result.SetStatus(ErrorCode.InvalidData, "提现已受理,加急请联系客服");
            }
            catch (Exception ex)
            {
                Yoyo.Core.SystemLog.Error("用户提现失败", ex);
